...
const hookPageEvents = async function (page, id) {
	page.on("console", (msg) => {
		...
		console.log(`${bold(`[T${id}]>`)} ${green(`console.log`.padEnd(17))} ${bold("|")} ${msg.text()}`);
	});
...
const hookWorkerEvents = async function (worker, id) {
	worker.client.on("Runtime.consoleAPICalled", (args) => {
		if (args.type != "log") return;
		console.log(`${bold(`[S${id}]>`)} ${green(`console.log`.padEnd(17))} ${bold("|")} ${args.args[0].value}`);
	});
}
