...
def rank_for_username(username):
    key = b"\x01" * 16
    G = GCM(key, IV)
    ...
    for col in range(128):
        H_basis = 1 << col
        result = ghash_with_H(blocks, H_basis)
...
candidates = [""]
candidates += ["a" * k for k in range(1, 65)]
candidates += ["user" + str(i) for i in range(1, 40)]
for _ in range(200):
...
for name in candidates:
    r = rank_for_username(name)
    rank_map.setdefault(r, []).append((name, len(name)))

for rank in sorted(rank_map.keys(), reverse=True):
    print("rank", rank, "sample", rank_map[rank][:5])
